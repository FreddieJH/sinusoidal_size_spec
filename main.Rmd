...
lm_comparison_data %>% 
  write_csv("data/processed/lm_comparison.csv")
...
save_as_plot(plot = fig_2, filename = "fig_2", output_type = c("png", "eps", "pdf"), asp_ratio = 1.618/2)
...
fig_s1_1_plots <- 
  ...
  ggplot(aes(bin_mid %>% logsqrt2, norm_density%>% logsqrt2)) +
  geom_point(data = fig_s1_1_data %>% filter(type == "meio"), col = meio_col) +
  geom_point(data = fig_s1_1_data %>% filter(type == "macro"), col = macr_col) +
  geom_point(data = fig_s1_1_data %>% filter(type == "both"), col = both_col) +
  facet_wrap(~site_ord) + 
  xlab("log(body size)") +
  ylab("log(normalised abundance)") +
  # scale_x_continuous(trans = log_trans(base = sqrt(2)), 
...
fig_s1_1 <-
  ggdraw() + 
  draw_plot(fig_s1_1_plots) +
  geom_rect(aes(xmin = 0.65, xmax = 0.99, ymin = 0.42, ymax = 0.99), fill = "white") +
  draw_plot(fig_s1_1_map, x = 0.615, y = 0.432, width = 0.4, height = 0.55) +
  fig_s1_1_legend_graphics
