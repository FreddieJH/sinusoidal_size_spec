...
lm_output %>% 
  ...
  left_join(nlrq_output %>% 
              select(site_code, nlrq_AIC)) %>% 
  left_join(lm_output2 %>% 
              select(site_code, lm2_AIC)) %>% 
  filter(lm2_AIC <= lm_AIC, lm2_AIC <= nlrq_AIC)
...
lm_output %>% 
  ...
  left_join(lm_output2 %>% 
              select(site_code, lm2_AIC)) %>% 
  filter(lm_AIC <= lm2_AIC, lm_AIC <= nlrq_AIC)
...
lm_output %>% 
  ...
  left_join(lm_output2 %>% 
              select(site_code, lm2_AIC)) %>% 
  filter(nlrq_AIC <= lm_AIC, nlrq_AIC <= lm2_AIC)
...
edgar_dat <-
  tibble(
    sieve_mm = c(0.5, 0.71, 1.0, 1.4, 2.0, 2.8, 4.0, 5.6) %>% log(base = sqrt(2)),
       afdw = c(0.025, 0.062, 0.153, 0.38, 0.95, 2.39, 6.0, 14.9) %>% log)


lm(afdw~sieve_mm, data = edgar_dat)

edgar_dat %>% 
  ggplot(aes(sieve_mm, afdw)) +
  geom_point() + 
  stat_smooth()


logs <- (log(0.5)+log(0.71))/2
logs <- log(0.5)
exp((- 1.009) + (2.643)*log(2.5))
